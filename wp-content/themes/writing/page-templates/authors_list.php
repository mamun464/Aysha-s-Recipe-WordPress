...
$show_author_posts = asalah_post_option('show_author_posts_list');
?>
<h4 class="page-title screen-reader-text"><?php echo get_the_title(); ?></h4>
<main class="main_content writing_author_list <?php echo asalah_content_class($asalah_blogpage_id); ?>">
<?php
$args['fields'] = array( 'ID', 'display_name' );

if (asalah_post_option('asalah_author_ids_list') !== '') {
	$args['include'] = asalah_post_option('asalah_author_ids_list');
}
if (asalah_post_option('asalah_author_orderby_list') !== '') {
	$args['orderby'] = asalah_post_option('asalah_author_orderby_list'); //  'registered', 'display_name', 'post_count'
}
...
if (get_avatar( get_the_author_meta( 'user_email', $author_id ), 80 ) != '') {
	$author_avatar = asalah_filter_lazyload_images(get_avatar( get_the_author_meta( 'user_email', $author_id ), 80 ));
}
...
if ($show_author_posts) {
	...
	echo asalah_single_related_posts($args, $title);
	}
}
?>
