...
if (asalah_option('asalah_lazyload_iframe_banner') == true) {
  $src = 'data-src';
  $lazyclass = 'lazyload ' . asalah_option('asalah_lazyload_effect');
}

if (asalah_option('asalah_lazyload_image_banner') == true) {
	function asalah_filter_gallery_img_atts( $atts, $attachment, $size ) {
			...
			$atts['class'] .= ' lazyload ' . asalah_option('asalah_lazyload_effect');
	    return $atts;
	}
	add_filter( 'wp_get_attachment_image_attributes', 'asalah_filter_gallery_img_atts', 10, 3 );
}
...
function asalah_video_frame($prov, $vid) {
    ...
    if ($prov == 'youtube') {
        ?>
          <<?php echo esc_attr($asalah_frame_tag); ?>  class="video_frame <?php echo esc_attr($lazyclass);?>" <?php echo esc_attr($src);?>="//www.youtube.com/embed/<?php echo esc_attr($vid); ?>?wmode=transparent&wmode=opaque" frameborder="0" allowfullscreen></<?php echo esc_attr($asalah_frame_tag); ?>>
        <?php
    } elseif ($prov == 'youtu') {
        ?>
        <<?php echo esc_attr($asalah_frame_tag); ?>  class="video_frame <?php echo esc_attr($lazyclass);?>" <?php echo esc_attr($src);?>="//www.youtube.com/embed/<?php echo esc_attr($vid); ?>" frameborder="0" allowfullscreen></<?php echo esc_attr($asalah_frame_tag); ?>>
        <?php
    } elseif ($prov == 'vimeo') {
        ?>
        <<?php echo esc_attr($asalah_frame_tag); ?>  class="video_frame <?php echo esc_attr($lazyclass);?>" <?php echo esc_attr($src);?>="//player.vimeo.com/video/<?php echo esc_attr($vid); ?>?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></<?php echo esc_attr($asalah_frame_tag); ?>>
        <?php
    } else {
...
function asalah_blog_post_banner($size = "", $class = "") {
    ...
    if (get_post_format() == "image" ) {
        ...
        if (get_post_meta($post->ID, '_format_video_embed', true) != '' ) {
            ...
            echo '</div>';
						wp_enqueue_script('asalah-fitvids-script');
        }else{
            // if video form empty
        }

    } elseif (get_post_format() == "gallery") {
			if (!is_single() && asalah_option('asalah_blog_gallery_crop') == 'no') {
				$size = 'full';
			}
      if (((class_exists( 'Jetpack' )) && (Jetpack::is_module_active( 'carousel' )))) {
        ...
        if (strpos($sound_url, $asalah_frame_tag) != false) {
            ...
            ?>
            <<?php echo esc_attr($asalah_frame_tag); ?> height="166" <?php if ($lazyclass != '') {?>class="lazyload"<?php } ?> style="overflow:hidden;border:none;width:100%" <?php echo esc_attr($src);?>="https://w.soundcloud.com/player/?url=<?php echo esc_url($sound_url); ?>"></<?php echo esc_attr($asalah_frame_tag); ?>>
            <?php
        }
        echo '</div>';
    } else {
