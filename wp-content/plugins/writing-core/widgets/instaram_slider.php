...
class Asalah_InstagramSlider extends WP_Widget {
		 ...
		 */
		 public function public_enqueue() {
	 		...
	 		wp_register_style( 'instag-slider', plugin_dir_url( __FILE__ ) . 'assets/css/instag-slider.css', array(), self::VERSION );
			wp_register_script( 'jr-inta-feed-script', plugin_dir_url( __FILE__ ) . 'assets/js/instagramfeed.js' , array( 'jquery' ), self::VERSION, true );
	 		wp_register_script( 'jquery-pllexi-slider', plugin_dir_url( __FILE__ ) . 'assets/js/jquery.flexslider-min.js', array( 'jquery' ), '2.2', false );
	 	}
		...
		public function admin_enqueue( $hook ) {
			...
			wp_register_style( 'jr-insta-admin-styles', plugin_dir_url( __FILE__ ) . 'assets/css/jr-insta-admin.css' , array(), self::VERSION );
			wp_register_script( 'jr-insta-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/jr-insta-admin.js' , array( 'jquery' ), self::VERSION, true );
		}
		...
		public function form( $instance ) {
			...
			wp_enqueue_style( 'jr-insta-admin-styles');
			wp_enqueue_script( 'jr-insta-admin-script');
				...
				<div class="jr-image-options">
					<h4 class="jr-advanced-title"><?php _e( 'Help us, help you!', 'jrinstaslider'); ?></h4>
						...
						<div class="donate-buttons">
							<a target="_blank" rel="nofollow noreferrer" title="Donate with Paypal!" href="http://bit.ly/2EseW2p"><p class="donate"><span></span>Donate with Paypal!</p></a>
						</div>
					</div></div>
				</div><br>
				<?php
		...
		public function instagram_images( $args ) {
			wp_enqueue_style( 'instag-slider');
			$php_images_data = $this->display_images( $args );

				wp_enqueue_script( 'jr-inta-feed-script');
				if ( $args['template'] == 'slider' || $args['template'] == 'slider-overlay' ) {
					wp_enqueue_script('jquery-pllexi-slider');
				}
				echo $php_images_data;
		}
		...
		private function display_images( $args ) {
			...
			if ($image_size == 'thumbnail') {
				...
				$image_size = '2';
			} else if ($image_size == 'jr_insta_square') {
				$image_size = '4';
			} else {
				$image_size = 'full';
			}
			...
			if (isset($image_lazyclass)) {
				$output = "<div class='lazyload {$images_div_class}' data-lazyscript='yes'>\n";
			} else {
				$output = "<div class='{$images_div_class}'>\n";
			}

			$output  .= "<ul id='list_".$widget_id."' class='no-bullet {$ul_class}'>\n";
			...
			if ( isset ( $args['search_for'] ) && $args['search_for'] == 'hashtag' ) {
				$username_arg = "'hashtag': '".$hashtag."',";
			} else {
				$username_arg = "'username': '".$username."',";
			}
			$link_to   = isset( $args['link_to'] ) ? $args['link_to'] : false;
			$nopin         = ( 1 == $args['no_pin'] ) ? 'nopin="nopin"' : '';
			if ($image_size == 'full' || !isset($image_size)) {
				$url_source = "var url = media[i].node.display_url;\n";
			} else {
				$url_source = "var url = media[i].node.thumbnail_resources[".$image_size."].src;\n";
			}

			if ( 'image_link' == $images_link ) {
				$link_attr = "var link = 'https://www.instagram.com/p/' + media[i].node.shortcode;\n";
			} elseif ( 'user_url' == $images_link ) {
				$link_attr = 'var link = "https://www.instagram.com/' . $username . '/";\n';
			} elseif ( 'image_url' == $images_link ) {
				$link_attr = "var link = media[i].node.display_url;\n";
			} elseif ( 'custom_url' == $images_link ) {
				$link_attr = "var link = '".$custom_url."'";
			} else {
				$link_attr = '';
			}

			if ( $images_link ) {
				$image_list_start = 'images += \'<a href="\' + link + \'" target="_blank"';

				if ( ! empty( $args['link_rel'] ) ) {
					$image_list_start .= ' rel="' . $args['link_rel'] . '"';
				} else {
					$image_list_start .= ' rel="nofollow noreferrer"';
				}

				if ( ! empty( $args['link_class'] ) ) {
					$image_list_start .= ' class="' . $args['link_class'] . '"';
				}
				$image_list_start .= ' title="\' + shortcaption + \'">\';';
				$image_list_end = 'images += \'</a>\';';
			} else {
				$image_list_start = '';
				$image_list_end = '';
			}
			...
			if ( $template == 'slider' || $template == 'slider-overlay' ) {
				$caption_list .= 'if (typeof caption !== "undefined") {';

					if ( $template == 'slider-overlay' ) {
						$caption_list .= "images += '<div class=\'jr-insta-wrap\'>';\n";
					}

					$caption_list .= "images += '<div class=\'jr-insta-datacontainer\'>';\n";
					if (in_array( 'time', $args['description'] )) {
						$caption_list .= 'if (typeof time !== "undefined") {';
							$caption_list .= "images += '<span class=\'jr-insta-time\'>'+time+' ago</span>';\n";
						$caption_list .= '};';
					}
					if ( $username && in_array( 'username', $args['description'] )) {
						$caption_list .= "images += '<span class=\'jr-insta-username\'>by <a rel=\'nofollow noreferrer\' href=\'https://www.instagram.com/{$username}/\' target=\'_blank\'>{$username}</a></span>';\n";
					}
					if ( in_array( 'caption', $args['description'] ) ) {
						$caption_list   .= "images += '<span class=\'jr-insta-caption\'>'+caption+'</span>';\n";
					}

					$caption_list .= "images += '</div>';\n";
					if ( $template == 'slider-overlay' ) {
						$caption_list .= "images += '</div>';\n";
					}
        ...
        jQuery(window).on('load', function(){";
					if (isset($image_lazyclass)) {
						$output .= "document.addEventListener('lazybeforeunveil', function(e){
                ...
                'callback': function(data){
										...
										var images = '';
										var max = {$images_number};
										if (media.length < {$images_number}) {
											max = media.length;
										}
										if (media instanceof Array) {
											for (i=0; i < max; i++) {\n
													...
													time = timeSince(time);\n
													if ('node' in media[i].node.edge_media_to_caption.edges) {
													...
													var caption = description.replace( '/@([a-z0-9_]+)/i', '&nbsp;<a href=\"https://www.instagram.com/$1/\" rel=\"nofollow noreferrer\" target=\"_blank\">@$1</a>&nbsp;' );\n
													caption = caption.replace( '/\#([a-zA-Z0-9_-]+)/i', '&nbsp;<a href=\"https://www.instagram.com/explore/tags/$1/\" rel=\"nofollow noreferrer\" target=\"_blank\">$0</a>&nbsp;');\n
													" . $link_attr . "
											...
											jQuery('#list_".$widget_id."').html(images);
										}
								}
					});";
					...
					"	if (jQuery('.instaslider-nr-{$widget_id}').length > 0) {" . "\n" .
					"		jQuery('.instaslider-nr-{$widget_id}').pllexislider({" . "\n" .
					"			animation: '{$animation}'," . "\n" .
		...
		private function get_template( $template, $args ) {
			...
			$src = isset($image_src) ? 'data-src' : 'src';
			$class= isset($image_lazyclass) ? 'class="' . $image_lazyclass . '"': '';

			$image_src = '<img ' . $src . '="' . $image_url . '" ' . $class . ' alt="' . $short_caption . '" title="' . $short_caption . '" '.  $nopin . '/>';
			...
			if ( $link_to ) {
				...
				if ( ! empty( $args['link_rel'] ) ) {
					...
					$image_output .= ' rel="nofollow noreferrer"';
				}
			...
			if ( $template == 'slider' ) {
				...
				if ( is_array( $args['description'] ) && count( $args['description'] ) >= 1 ) {
					...
					if ( in_array( 'username', $args['description'] ) && $username ) {
						$output .= "<span class='jr-insta-username'>by <a rel='nofollow noreferrer' href='https://www.instagram.com/{$username}/' target='_blank'>{$username}</a></span>\n";
					}
		...
		function powered_by_link( $support_author ) {
			...
			if ( $support_author ) {
				...
				if ( isset( $link['text'] ) && !empty( $link['text'] ) && isset( $link['domain'] ) && !empty( $link['domain'] ) ) {
					$link = '<div style="clear:both;text-align:right;font-size:10px;" >Powered by <a href="'.$link['domain'].'" rel="nofollow noreferrer" target="_blank">'.$link['text'].'</a></div>';
		...
		public function insta_attachment_fields( $form_fields, $post ) {
			...
			if ( !empty( $instagram_username ) ) {
				...
				$form_fields["jr_insta_username"] = array(
					...
					"input" => "html",
					"html"  => "<span style='line-height:31px'><a rel='nofollow noreferrer' target='_blank' href='https://www.instagram.com/{$instagram_username}/'>{$instagram_username}</a></span>"
				);
				...
				if ( !empty( $instagram_link ) ) {
					$form_fields["jr_insta_link"] = array(
						...
						"input" => "html",
						"html"  => "<span style='line-height:31px'><a rel='nofollow noreferrer' target='_blank' href='{$instagram_link}'>{$instagram_link}</a></span>"
					);
				}
